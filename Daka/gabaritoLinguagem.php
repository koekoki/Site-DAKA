...
<html>
   ...
   <header style="height: 100px;">
		...
		<div class = "container">
			...
			$r= "SELECT DISTINCT resumo from alternativa where IdQuestao between 106 and 135";
            $resumao = mysqli_query($connect,$r);

			$r1= "SELECT DISTINCT resumo from alternativa where IdQuestao between 106 and 135";
            $resumao1 = mysqli_query($connect,$r1);

            $q1= "SELECT resposta from alternativa where Certa = 1 and IdQuestao between 106 and 135 ";
            $ansresults = mysqli_query($connect,$q1);

			$q2= "SELECT Enunciado from questao where IdQuestao BETWEEN 106 AND 135";
            $ansresults2 = mysqli_query($connect,$q2);
	
            $i = 106;

			$q3= "SELECT resposta from alternativa where Certa = 1 and IdQuestao between 156 and 170 ";
            $ansresults3 = mysqli_query($connect,$q3);

			$q4= "SELECT Enunciado from questao where IdQuestao between 156 and 170";
				  ...
				  $flag = 1 == $selected[$i];
						  ...
						  if($flag){
							  ...
							  <?php
								...
								$sql = "insert into historico (Email, IdQuestao, Acertou, idArea) values ('$email', $i, '1', '4')";
							...
							<li class="espaçoEnunciado"><p class = "EnunciadoErrado"><?php echo $rows1['Enunciado'];?><br></p>
							  ...
							  <?php
							    $email = $_SESSION['Email'];
								$sql = "insert into historico (Email, IdQuestao, Acertou, idArea) values ('$email', $i, '0', '4')";
								$salvar = mysqli_query($connect, $sql);
						  }					
					  $i++;		
				  }
					...
					$resumo1 = mysqli_fetch_array($resumao1);
					  $flag = 1 == $selected[$j];
							  ...
							  if($flag){
								  ...
								  <?php
								    $email = $_SESSION['Email'];
									$sql = "insert into historico (Email, IdQuestao, Acertou, idArea) values ('$email', $j, '1', '4')";
									$salvar = mysqli_query($connect, $sql);
