...
<html lang="pt-br"></html>
    ...
    font-weight: bolder;">  <?php  if(isset($_SESSION['Email'])){
        ...
        <h2 class="title title-second">Exatas</h2>
        <div class = "container">
...
$q3= "SELECT COUNT(IdQuestao) FROM historico WHERE Email LIKE '$Email' and idArea = 2";
$ansresults3 = mysqli_query($connect,$q3);
$certa= "SELECT COUNT(Acertou) FROM historico WHERE Email LIKE '$Email' and Acertou = 1 and idArea = 2";
...
while( $totalQuestoes = mysqli_fetch_array($ansresults3)){
 ...
 <p class = "Total"> <?php echo $totalQuestoes['COUNT(IdQuestao)']." Questões respondidas<br> totais de acerto: ".$i['COUNT(Acertou)']; }?></p>
            		...
            		$q= "SELECT *  FROM historico where Email Like '$Email' and idArea =2";
                    $ansresults = mysqli_query($connect,$q);
                    
                    while($rows = mysqli_fetch_array($ansresults)){
                    $number = $rows['IdQuestao'];
                    $q2= "SELECT * FROM questao where IdQuestao = $number and idArea = 2";
                    $ansresults2 = mysqli_query($connect,$q2);
                    $q4= "SELECT * FROM historico where IdQuestao = $number and Email LIKE '$Email' and idArea = 2";
